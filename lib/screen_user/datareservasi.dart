...
class _DataReservasiState extends State<DataReservasi> {
  ...
  Color getColorForDate(DateTime date) {
    ...
    if (bookings != null && bookings.isNotEmpty) {
      int acceptedCount = bookings.where((booking) => booking['status'] == 'Accepted').length;

      bool sesiFullDayAccepted = bookings.any((booking) =>
              booking['session'] == 'FULL') ||
          (bookings.any((booking) =>
                  booking['session'] == 'PAGI') &&
              bookings.any((booking) =>
                  booking['session'] == 'SORE'));

      print(sesiFullDayAccepted);
      if (sesiFullDayAccepted) {
        return Colors.red;
      } else if (acceptedCount > 0) {
        return Colors.yellow; // Jika ada sesi yang diterima (sesi pagi/siang)
      }
    }

    return Colors.green; // Jika tidak ada sesi yang diterima
  }
                ...
                return Padding(
                  ...
                  child: Column(
                    children: [
                      TableCalendar(
                        ...
                        calendarStyle: CalendarStyle(
                          todayDecoration: BoxDecoration(
                            color: Colors
                                .transparent, // No special decoration for today
                        ...
                        calendarBuilders: CalendarBuilders(
                          defaultBuilder: (context, date, _) {
                            List<dynamic>? bookings = acceptedBookings[
                                DateTime(date.year, date.month, date.day)];
                            Color dateColor =
                                bookings != null && bookings.isNotEmpty
                            ...
                            return GestureDetector(
                              onTap: () {
                                ...
                                List<dynamic>? bookings =
                                    acceptedBookings[dateOnly];

                                // Cek apakah ada sesi yang sudah diterima
                                bool hasAcceptedBookings = bookings?.any(
                                        (booking) =>
                                            booking['status'] == 'Accepted') ??
                                    false;

                                if (hasAcceptedBookings) {
                                  // Hanya tampilkan detail jika ada sesi yang diterima
                                  List<dynamic> acceptedBookings = bookings!
                                      .where((booking) =>
                                          booking['status'] == 'Accepted')
                                      .toList();
                                ...
                                // Jika tidak ada sesi yang diterima, tidak melakukan apa-apa
                              },
                              child: Container(
                      ...
                      SizedBox(height: 6),
                      _buildLegendItem(
                          Colors.yellow, 'Sudah dipesan satu sesi'),
                      SizedBox(height: 6),
